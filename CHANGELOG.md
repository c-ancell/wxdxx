...
- WFO details now show major cities in coverage area for all 122 WFOs (#11)

### Performance

- LRU cache now uses O(1) OrderedDict instead of O(n) list operations
- User-Agent version now dynamically fetched from package metadata
...
- Map now displays zones for Severe Thunderstorm Warnings and other county-based alerts
- Zone geometry lookup now tries forecast, county, and fire zone endpoints

## [0.8.14] - 2025-12-28

### Fixed

- Tornado Watches now correctly labeled instead of showing as Severe Thunderstorm Watch
- Watch type now parsed from actual product text content

## [0.8.13] - 2025-12-28

### Fixed

- App no longer hangs when viewing Tornado Watch or other large-area alerts
- Limited zone station queries to 15 zones max to avoid excessive API calls

## [0.8.12] - 2025-12-28

### Fixed

- WFO sidebar now shows severe weather warnings that appear in news ticker (#27)
- Now fetches both forecast and county zones for each WFO
...
- News ticker no longer shows invalid WFO codes like "LIN" derived from city names (#28)
- Tornado Watches now display as red instead of yellow/orange (#29)
- Severe Thunderstorm Watches now display as dark goldenrod instead of same color as warnings

## [0.8.9] - 2025-12-28

### Fixed

- METAR wind speeds now check unitCode before converting units (#22)
- Added senderName fallback for WFO attribution when senderCode and zone lookup fail (#25)
